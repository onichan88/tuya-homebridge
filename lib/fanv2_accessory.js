...
class Fanv2Accessory extends BaseAccessory {
  ...
  addLightService() {
    this.lightStatus = this.statusArr.find((item, index) => { return item.code === 'switch_led' && typeof item.value === 'boolean' });
    if (this.lightStatus) {
      // Service
  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (const statusMap of statusArr) {
      ...
      if (this.lightService && statusMap.code === 'switch_led') {
      ...
      if (this.lightService && statusMap.code === 'temp_value') {
        ...
        this.temp_range = this.getColorTempFunctionRange(this.tempValue.code)
        const rawValue2 = this.tempValue.value;
        const percentage2 = Math.floor((rawValue2 - this.temp_range.min) * 100 / (this.temp_range.max - this.temp_range.min)); //    $
        this.normalAsync(Characteristic.ColorTemp, percentage2 > 100 ? 100 : percentage2, this.lightService)
      }
    }
  }
  ...
  getSendParam(name, hbValue) {
    ...
    switch (name) {
      ...
      case Characteristic.Brightness:
        ...
        break;
      case Characteristic.ColorTemp:
        value = Math.floor((this.temp_range.max - this.temp_range.min) * hbValue / 100 + this.temp_range.min); //  value 0~100
  ...
  getColorTempFunctionRange(code) {
    if (this.functionArr.length == 0) {
      return { 'min': 10, 'max': 1000 };
    }
    var funcDic = this.functionArr.find((item, index) => { return item.code === code })
    if (funcDic) {
      let valueRange = JSON.parse(funcDic.values)
      let isnull = (JSON.stringify(valueRange) == "{}")
      return isnull ? { 'min': 0, 'max': 1000 } : { 'min': parseInt(valueRange.min), 'max': parseInt(valueRange.max) };
    } else {
