...
class Fanv2Accessory extends BaseAccessory {
  ...
  addLightService() {
    this.lightStatus = this.statusArr.find((item, index) => { return (item.code === 'switch_led' || item.code === 'light') && typeof item.value === 'boolean' });
    if (this.lightStatus) {
      // Service
  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (const statusMap of statusArr) {
      ...
      if (this.lightService && (statusMap.code === 'switch_led' || statusMap.code === 'light')) {
